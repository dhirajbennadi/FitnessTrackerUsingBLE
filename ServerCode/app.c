...
SL_WEAK void app_init(void)
{
  ...
  /*Configuration of Oscillator for LETIMER0*/
  enableOscillator();
  selectClock();
  setClockPrescaler();
  enableClock();
  ...
  /*Configuration of LETIMER0*/
  initLETIMER();

  /* Setting CURRENT_ENERGY_MODE EM1 and EM2 for Power Manager*/
   if (CURRENT_ENERGY_MODE == EM1){
       sl_power_manager_add_em_requirement(SL_POWER_MANAGER_EM1);
     }
   else if(CURRENT_ENERGY_MODE == EM2){
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  if(shiftStateMachine == 0)
    {
      //val = GPIO_PinInGet(I2C_TEMP_SENSOR_ENABLE_PORT , 15);

      //displayPrintf(DISPLAY_ROW_ACTION, "Value = %d", val);
      //LOG_INFO("Value = %d\n\r", val);
      IMUStateMachine(evt);
    }
  else
    {
      //displayPrintf(DISPLAY_ROW_ACTION, "Some Action Happening 2\n");
      //val = GPIO_PinInGet(I2C_TEMP_SENSOR_ENABLE_PORT , 15);
      //displayPrintf(DISPLAY_ROW_ACTION, "Value = %d", val);
      //LOG_INFO("Value = %d\n\r", val);
      IMUStateMachineForReadingValues(evt);
    }
  //TempStateMachine(evt);

#else
  DiscoveryStateMachine(evt);
#endif
